  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">收入阶段</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">理财诉求</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">个人财务健康与无痛攒钱法则指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 个人财务记账与消费分析专家');
...
const metricsList = [
  {
    "key": "expenseCategorizationRigor",
    "label": "收支分类严谨度"
  },
  {
    "key": "impulseExpenseWarning",
    "label": "冲动消费预警度"
  },
  {
    "key": "savingsGoalFeasibility",
    "label": "储蓄目标可行度"
  },
  {
    "key": "cashFlowHealthIndex",
    "label": "现金流健康指数"
  },
  {
    "key": "budgetControlEffectiveness",
    "label": "预算管控有效性"
  }
];
const guideRules = [
  {
    "title": "532 预算黄金法则",
    "advice": "收入50%用于衣食住行必要开支，30%用于品质生活，20%强制存入防风险账户",
    "avoid": "切忌先花钱再存剩余，应先存钱再消费"
  },
  {
    "title": "冲动消费冷静期",
    "advice": "购买非刚需物品前建立 72 小时冷静期清单，问自己 3 个购买理由",
    "avoid": "切勿因打折促销或满减凑单盲目消费"
  },
  {
    "title": "财务安全垫底线",
    "advice": "随时保持 3 至 6 个月月度必要开支的活期现金充当紧急备用金",
    "avoid": "严禁将所有积蓄投入高风险股票投资中"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[JIZHANG_SCORES\][\s\S]*?\[\/JIZHANG_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[JIZHANG_SCORES\](.*?)\[\/JIZHANG_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('jizhang_history_records');
...
const saveHistory = () => {
  localStorage.setItem('jizhang_history_records', JSON.stringify(historyList.value));
};
